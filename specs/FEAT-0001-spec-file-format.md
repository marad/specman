...
1. **YAML frontmatter** carrying structured metadata. Required: `id`, `title`, `status`, `depends_on`. The `status` field accepts values from the set `{draft, active, shipped, deprecated}`. Status is a human-managed label for filtering and communication — SpecMan does not enforce transitions or change it automatically. `specman new` sets `draft` as the default; the author updates it manually as the spec progresses. The `depends_on` field lists specs whose implementation this spec builds on — "implementing this spec assumes its dependencies are already implemented." SpecMan uses it mechanically: the validator checks that targets exist and the graph is acyclic, and `specman sync` (without an id) processes drifted specs in dependency order. Optional: `platforms` — a list of strings naming the platforms this spec targets (e.g. `[web, ios, android]`). Platform values are project-defined and not enumerated by the validator.
2. **A markdown body** with conventional sections in this canonical order: `## Intent`, `## Behavior`, `## Constraints`, `## Examples`, `## Acceptance criteria`, `## Out of scope`, `## Non-goals`, `## Open questions`. Authors may add additional sections beyond the canonical set (e.g. `## Security considerations`, `## Migration plan`); they are preserved by the parser and serializer and not flagged by the validator.
...
Acceptance criteria use stable IDs of the form `AC-N:` so that commit trailers and agent plans can reference specific criteria even as surrounding prose changes. Acceptance criteria are the sole interface between a spec and the sync loop (FEAT-0004). Only AC changes drive implementation work. The remaining body sections provide context for human readers and for the agent during planning, but do not independently trigger code changes. When a non-AC edit has implementation consequences, the author must reflect it in an acceptance criterion; otherwise the change is editorial and is sealed without a full sync (see FEAT-0004). An AC may carry a platform marker (e.g. `AC-5 *(web only)*:`) when a criterion applies only to a subset of the spec's declared `platforms`. Files are named `<FEAT-ID>-<slug>.md`. They may live directly under `specs/` or in any subfolder of `specs/` — subfolder organization is a free choice for readability and not part of identity. `<FEAT-ID>` values are globally unique across the whole `specs/` tree regardless of where a file lives, and `depends_on` references resolve by id alone. Moving a spec between folders never changes its identity or invalidates references to it. The `specs/assets/` subtree is reserved for non-markdown assets (see below) and is not treated as a container for specs.
